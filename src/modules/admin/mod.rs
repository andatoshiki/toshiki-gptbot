...
async fn public(
    ...
    match member_mgr.public_usable(value).await {
        Ok(_) => {
...
async fn adduser(
    ...
    match member_mgr.adduser(username).await {
        Ok(value) => {
            bot.send_message(
                msg.chat.id,
                if value {
...
async fn deluser(
    ...
    match member_mgr.deluser(username).await {
        Ok(value) => {
            bot.send_message(
                msg.chat.id,
                if value {
...
impl Module for Admin {
    ...
    fn commands(&self) -> Vec<Command> {
        ...
        vec![
            Command::new("public", " (admin) Publicize bot to public by granting uniersal access rights", dptree::endpoint(public)),
            Command::new("adduser", "(admin) Add an admin or uesr to grant access permission to the target user by username", dptree::endpoint(adduser)),
            Command::new("deluser", "(admin) Remove an existing bot user or admin from the permitted name list for resitricting access", dptree::endpoint(deluser)),
        ]
    }
}
